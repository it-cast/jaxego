...
| SUG-008 | Completar o guard de "entregas ativas" em `remove_neighborhood` contra a tabela `deliveries` na Phase 7 | Phase 6 (Plan 02): a tabela de entregas ainda não existe no M1; o guard usa cobertura/preço como proxy de "em uso". Quando `deliveries` chegar (Phase 7), a checagem deve passar a consultar entregas ativas no bairro | proposta (descoberta na execução da Phase 6) |
| SUG-009 | Migrar `jx-kyc-queue-table` (Phase 5) para consumir o novo `jx-data-table` | Phase 6 (Plan 04): o primitivo governado `jx-data-table` generaliza a fila KYC; migrar sem redesenho de colunas reduz duplicação. Não obrigatório (a fila atual funciona) | proposta (descoberta na execução da Phase 6) |
