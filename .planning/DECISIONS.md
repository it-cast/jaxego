...
| DEC-003 | **OQ-3 (contrato Safe2Pay) resolvida como SUPOSIÇÃO DOCUMENTADA** para destravar a Phase 10. Assume-se: (a) split/marketplace DISPONÍVEL no plano contratado; (b) escrow interno de 24h pós-FINALIZADA MANTIDO (ADR-009 v2) independente do prazo de repasse do PSP; (c) taxa por transação parametrizável (não hardcoded). TODA a integração Safe2Pay nasce com `[ASSUMIDO]` e atrás de interface própria (trocar de PSP/ajustar escrow não pode doer — ADR-009 v2). **Revisar quando o contrato real confirmar split/prazo/taxa.** | Dono optou por avançar com suposições no fim do MS-03 (2026-06-11) em vez de bloquear; pagamento atrás de interface própria mitiga retrabalho | Phases 10-12 codam com adapter + stub + valores parametrizados; abrir ADR de confirmação quando o contrato chegar | 2026-06-11 |
| DEC-005 | **Separação física dos apps + lib compartilhada (ADR-014).** Estrutura passa a ser `apps/api` (Python), `apps/web` (Angular: admin+loja), `apps/app` (Ionic: entregador), `packages/shared` (design system jx-* + core/auth + tokens + login/tracking cross-app). Monorepo **npm workspaces** com `node_modules` hoisted na raiz; aliases `@jaxego/shared/*` e `@jaxego/core/*`. Imports cross-app proibidos (o comum vive na lib). | Dono pediu explicitamente a topologia api/web/app e confirmou a lib compartilhada como melhor estrutura (2026-06-17); a v1.0 só separava por build target (postmortem). | `apps/web` reduzido a admin+loja; `apps/app` criado; `core/shared/styles/app/login/tracking/br-format/delivery.models` movidos p/ a lib. CI: `npm ci` na raiz, jobs `web`+`app`, zero-hex cobre lib+apps, APK de `apps/app`. Verde local: 4 builds + lint + testes (web 54, app 31). Deploy backend inalterado. | 2026-06-17 |
| DEC-004 | **Resequenciamento: parte financeira pesada de Safe2Pay movida para PÓS-DEPLOY.** A ex-Phase 11 (fatura mensal, resolução de disputas com bloqueio 90d, saques, conciliação diária) vira **Phase 15** (novo MS-06), a **última** do projeto, a ser construída/ativada **depois** do deploy do piloto. Nova ordem de build: 12 → 13 → 14 (deploy) → 15. Consequências: (a) Phase 13 religada de depende-de-[11] para [9] — a UI admin de disputas/suspensão se apoia no primitivo `payment_dispute` já entregue na Phase 9; a resolução financeira da disputa vai para a Phase 15. (b) No deploy (Phase 14) ficam live assinatura + cartão/PIX com split + escrow 24h (Phase 10), **assumindo contrato Safe2Pay assinado até o deploy** (dono escolheu cartão/PIX live no launch). (c) A regressão de pagamentos da Phase 14 cobre só o escopo da Phase 10. (d) MS-04 fecha com checkout (10) + API (12); MS-06 (Phase 15) é pós-piloto. | Dono pediu (2026-06-11): "siga no autopilot fechando todas as MS e deixando a parte financeira safe2pay pra última fase, pós deploy". Mantém o piloto destravado sem depender do back-office financeiro completo; checkout (Phase 10) já estava pronto em dev/test | Autopilot constrói 12, 13, 14, depois 15. Phase 15 código nasce verde com Stub (DEC-003); ativação de produção atrás de TD-10-01..04 (contrato). MILESTONES.md e STATE.md atualizados | 2026-06-11 |
