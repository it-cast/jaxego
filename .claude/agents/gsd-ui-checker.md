---
name: gsd-ui-checker
description: Validates UI-SPEC.md design contracts against 7 quality dimensions (incl. wireframe fidelity). Produces BLOCK/FLAG/PASS verdicts. Spawned by /gsd-ui-phase orchestrator.
...
- A wireframe exists for the screen but the spec silently ignores or diverges from it
...
## Dimension 7: Wireframe Fidelity (v0.9.7)

**Question:** When a wireframe exists for a screen, does the UI-SPEC provably cover it — every structural element accounted for, every deviation declared with a reason?

**Procedure:**
1. For each screen in the UI-SPEC, read its `wireframe_source:` declaration.
2. If `wireframe_source` is an HTML/JSX/TSX/Vue/Svelte file: run
   `node .claude/get-shit-done/bin/gsd-tools.cjs wireframe-contract <path>`
   and diff the contract against the spec — do NOT trust the spec's self-report.
3. For each contract item (region, heading, interactive element, form input,
   state, nav_target): it must appear in the screen's spec OR in the screen's
   `deviations:` block with an explicit reason.

**BLOCK if:**
- A screen has a matching wireframe in `projeto/wireframes/` but the spec has no `wireframe_source:` declaration for it (silent ignore of source material)
- Contract items missing from BOTH spec and deviations (silent omission — the exact failure mode this dimension exists to prevent)
- A deviation entry has no reason ("mudamos" is not a reason; "dropdown→radio por accessibility-pro §3.2" is)
- Wireframe colors mapped to nonexistent tokens (cross-check with Dimension 3 / Gate 2)

**PASS if:**
- Every contract item is covered or deviated-with-reason
- `nav_targets` from the contract resolve to real routes in the spec
- Image-only wireframes (.png/.jpg/.pdf): `wireframe_source` declared + deviations block present (item-level check is N/A — no mechanical contract from images)

**N/A if:**
- Screen declares `wireframe_source: none` and no matching file exists in `projeto/wireframes/` (verify the absence — declaring none while a matching wireframe exists is a BLOCK)

**Example issues:**
```yaml
dimension: 7
severity: BLOCK
description: "Wireframe contract for 'nova-previsao' has button 'Rodar Simulação' and 3 form inputs; spec covers the inputs but the button is absent from both spec and deviations"
fix_hint: "Add the button to the screen spec, or add a deviations entry with the reason it was removed"
```
```yaml
dimension: 7
severity: PASS
description: "18/18 contract items covered (16 in spec, 2 in deviations with reasons); nav_targets map to declared routes"
```
...
| 7 Wireframe Fidelity | {PASS/FLAG/N-A} | {brief note} |
