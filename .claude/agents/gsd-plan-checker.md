...
@./.claude/get-shit-done/references/skills-enforcement.md
...
## Dimension 13: API Surface Compliance (v0.9.2)

**Question:** Do plan tasks reference functions/classes/endpoints with **signatures consistent** with their actual definition in the codebase?

**Origin (field data):**
- Alfie phase-26: `LLM factory auto-wrap` — executor adicionou wrap para gemini não previsto no plan
- Alfie phase-28: `Schemas assumidos incorretamente` — eval datasets criados sem ler o schema da tool (`query` vs `descricao_necessidade`, `cep_origem` vs `origem`, `tenant_id` que não existe)
- Rota Certa phase-08: `CR-01 — dead code com bug no cancellation_pct` — executor criou `_compute_and_persist_single` com assinatura divergente de `score_service.compute_and_persist_score`. Plan-checker validava existência da função citada, **não a assinatura**.

**Why this matters:** plan-checker hoje valida que função citada existe, não que assinatura está correta. Resultado: executor cria funções paralelas com assinaturas erradas, ou usa schemas inventados em vez do real. Bugs passam para o code review.

**Skip if:** PLAN.md não cita nenhuma função/classe/endpoint específico do codebase existente (greenfield phase pura).

**Process:**

### 13.1 Extract API references from PLAN.md

Procurar no PLAN.md por padrões que referenciam código existente:

```yaml
patterns_to_extract:
  - python_function_call: "{name}({arg1}, {arg2}, ...)"
  - python_method_call: "{obj}.{method}({args})"
  - python_class_use: "{ClassName}(...)"
  - typescript_function: "{name}({args})"
  - api_endpoint: "/{prefix}/{path}" or "router.{method}('{path}', ...)"
  - schema_field: "field {name} of {ModelName}"
  - file_path_imports: "from {module} import {names}"
```

Para cada referência:
1. Extrair `target_identifier` (nome da função/classe/endpoint/campo)
2. Tentar localizar definição real no codebase via Grep/Glob
3. Se não encontrar → não é referência real (greenfield), pular
4. Se encontrar → seguir para 13.2

### 13.2 Validate signature compatibility

Para cada referência localizada:

**Python functions:**
```bash
# Achar definição
grep -rn "def {name}(" backend/ src/ --include="*.py"
# Extrair assinatura
ast.parse() ou regex: "def {name}\((.*?)\)\s*(?:->.*?)?:"
```

**Comparar:**
- Nomes de parâmetros declarados vs usados no PLAN
- Quantidade de argumentos
- Argumentos obrigatórios (sem default) vs opcionais
- Type hints quando declarados

**TypeScript functions:**
```bash
grep -rn "function {name}\|const {name} = (\|{name} = (" frontend/src/ --include="*.ts"
```

**API endpoints:**
```bash
# Backend
grep -rn "router\.\(get\|post\|put\|delete\|patch\).*'{path}'" backend/
# Verificar path inteiro: "/api/v1/{...}" não "/v1/{...}" (Rota Certa phase-04 pegou)
```

**Schemas / models:**
```bash
# Pydantic / SQLAlchemy / TypedDict
grep -rn "class {ModelName}" backend/ src/ --include="*.py"
# Listar campos declarados vs campos referenciados no PLAN
```

### 13.3 Detect mismatches
...
1. **Function signature mismatch:**
   ```yaml
   issue:
     dimension: api_surface_compliance
     severity: blocker
     description: "PLAN.md calls score_service.compute_and_persist_score(courier_id) mas a função real espera (courier_id, score_value, computed_at)"
     plan_reference: "01-02-PLAN.md:78 — Task T3.2"
     actual_signature: "compute_and_persist_score(courier_id: UUID, score_value: float, computed_at: datetime) -> Score"
     plan_signature: "compute_and_persist_score(courier_id)"
     fix_hint: "Atualizar PLAN.md para chamar com 3 args, ou criar overload se assinatura precisa mudar (com ADR)"
   ```
...
2. **Schema field mismatch:**
   ...
   issue:
     dimension: api_surface_compliance
     severity: blocker
     description: "PLAN.md usa campos {query, cep_origem, tenant_id} mas tool schema declara {descricao_necessidade, origem} — tenant_id não existe"
     plan_reference: "02-04-PLAN.md:23 — Eval dataset"
     actual_schema: "{descricao_necessidade: str, origem: CEP}"
     plan_assumptions: "{query: str, cep_origem: str, tenant_id: UUID}"
     fix_hint: "Ler o arquivo .py da tool ANTES de criar dataset. Atualizar PLAN para usar nomes reais."
   ```
...
3. **Endpoint path mismatch:**
   ...
   issue:
     dimension: api_surface_compliance
     severity: blocker
     description: "PLAN.md descreve POST /v1/webhooks/safe2pay mas main.py monta router em /api/v1/"
     plan_reference: "04-02-PLAN.md:45"
     actual_path: "/api/v1/webhooks/safe2pay"
     plan_path: "/v1/webhooks/safe2pay"
     fix_hint: "Conferir prefix em main.py (app.include_router(..., prefix=...))"
   ```

4. **Dead code parallelism:**
   ```yaml
   issue:
     dimension: api_surface_compliance
     severity: warning
     description: "PLAN.md cria função _compute_and_persist_single mas função real compute_and_persist_score já existe — paralelismo suspeito"
     plan_reference: "08-02-PLAN.md:67"
     fix_hint: "Confirmar se nova função é necessária ou se deveria usar a existente"
   ```

### 13.4 Severity rules

- **BLOCKER**: PLAN referencia API/schema/endpoint com assinatura objetivamente incorreta (vai dar erro de runtime ou silenciosamente fazer coisa errada)
- **WARNING**: assinatura ambígua ou parcial (ex: PLAN omite parâmetros opcionais mas isso é OK)
- **INFO**: PLAN cria função paralela (`_xxx_single`) quando uma função canônica já existe (review humano decide)
...
issues:
  - severity: blocker
    type: function_signature_mismatch
    plan_file: "01-02-PLAN.md"
    plan_line: 78
    target: "score_service.compute_and_persist_score"
    expected: "compute_and_persist_score(courier_id: UUID, score_value: float, computed_at: datetime)"
    found_in_plan: "compute_and_persist_score(courier_id)"
    fix_hint: "..."

verdict: BLOCK | PROCEED_WITH_WARNINGS | PASS
```

**Cost note:** Dimension 13 só é executada se PLAN.md tem ≥3 referências a código existente. Greenfield phases puras pulam automaticamente.



**Question:** Are all mandatory skills cited in `## Skills Consultadas` of each PLAN.md based on phase context?

**Why this matters:** the framework has 44 skills, each with `triggers.yaml` declaring conditions under which it becomes mandatory. Without this check, anti-patterns the skill prohibits appear in code (hex hardcoded instead of tokens, JWT in localStorage, brazilian forms without CPF/CNPJ validation, etc.).

**Process:**

### 6.1 Build phase context vector

Collect signals from these sources:

```yaml
# From specs/project.yaml
locale: pt-BR  # or en-US, etc.

# From specs/stack.yaml
stack:
  backend_lang: python
  backend_framework: fastapi
  frontend_web_framework: angular
  frontend_web_ui_kit: angular-material
  frontend_mobile_framework: ionic
  database_primary: mysql
  llm_providers: ["claude", "openai"]
  payment_psp: stripe

# From SPRINT.md / phase metadata frontmatter
flags:
  has_ui: true
  has_ui_web: true
  has_mobile: false
...
# Inferred from phase goal + PLAN.md content
features: ["auth", "signup"]  # detect by keyword scanning
keywords_in_plan: ["JWT", "password hash", "form validation"]  # extracted from PLAN.md
file_paths_planned: ["backend/app/api/auth/login.py", "frontend/src/app/login/**"]
```

### 6.2 Walk skills directory and load triggers

```bash
# List all triggers.yaml
SKILL_TRIGGERS=$(find ./.claude/skills -name "triggers.yaml" -type f)

# Parse each
for trigger in $SKILL_TRIGGERS; do
  SKILL_DIR=$(dirname "$trigger")
...
### 6.3 Match phase context against each skill's `required_for` rules

For each skill, evaluate `required_for` clauses (OR logic — any clause matches → skill is required):

| Clause type | How to evaluate |
|-------------|-----------------|
| `flag: X` | True if `flags.X == true` in phase context (inclui has_ui, has_api, has_admin, has_ai, integration_check) |
| `locale: X` | True if `locale == X` |
| `stack: X` | True if any value in `stack.*` matches X |
| `feature: X` | True if X in `features[]` |
| `context: X` | True if context tag matches |
| `config: X` | True if config.json path X is truthy (ex.: `parallelization.task_level`) |
| `keyword_any: [...]` | True if any keyword found in phase goal, PLAN.md actions, or DoD |
| `path_pattern: [...]` | True if any planned file path matches glob |
| `phase_type: X` | True if phase type matches |
| `integration: X` | True if integration declared in stack.yaml |

Conditional sub-rules (`when_locale: X`, `when_handles_pii: true`, etc.) act as AND with the parent clause.

### 6.4 Build mandatory skills set

```
MANDATORY_SKILLS = []
for each skill:
  for each clause in skill.required_for:
    if clause matches phase context:
      MANDATORY_SKILLS.append(skill.name)
      break  # one match enough
```

### 6.5 Read PLAN.md `## Skills Consultadas` section

```bash
# Extract cited skills from each PLAN.md
for plan in "$PHASE_DIR"/*-PLAN.md; do
  # Look for ## Skills Consultadas section
  awk '/^## Skills Consultadas/,/^## /' "$plan" | grep -oE '`[a-z-]+/[a-z-]+`|`[a-z-]+`' | tr -d '`'
done
```

Also check `## Skills Dispensadas (com justificativa)` — skills explicitly dispensed with reason are acceptable.

### 6.6 Compute coverage gap

```
CITED_SKILLS = union(skills cited in any PLAN.md)
DISPENSED_SKILLS = union(skills in "Dispensadas" section, with non-empty justification)
COVERED = CITED_SKILLS ∪ DISPENSED_SKILLS

MISSING = MANDATORY_SKILLS - COVERED
```
...
**If `MISSING` is empty:**
- Pass dimension 6
- Log in result: `mandatory_skills_coverage: clear, mandatory_count: N, cited: M, dispensed: K`

**If `MISSING` has entries:**
- BLOCK with severity `blocker`
- Issue per missing skill:

```yaml
issue:
  dimension: mandatory_skills_coverage
  severity: blocker
  description: "Mandatory skill not cited based on phase context"
  skill: "ux-advanced/form-ux-mastery"
  triggered_by:
    - flag: has_forms
    - keyword_match: "form validation"
  fix_hint: |
    Add to ## Skills Consultadas section of PLAN.md:
    `ux-advanced/form-ux-mastery` — used for [reason from skill SKILL.md].

    Or add to ## Skills Dispensadas with justification why this phase
    does NOT need form-ux-mastery.
```

### 6.8 Quality of citation (info-level only, never blocker)

For cited skills, optionally check:
- Citation includes 1-line reason (not just bare name)
- Cited skills' SKILL.md exists (not phantom citation)

These are `info` issues, not blockers — citation alone is what gate 3 enforces structurally. **The "did Claude actually READ the skill" problem is not solvable here** — that requires the human-driven prompt described in CLAUDE.md Rule 5 ("force literal quote from SKILL.md before execution").
...
```markdown
## Skills Consultadas
- `ux-advanced/form-ux-mastery` — para validação de CEP/CNPJ inline e erros progressivos no signup
- `br/brazilian-forms` — máscaras CPF/CNPJ/CEP brasileiras
- `quality/error-ux-patterns` — copy de erro recuperável vs. grave

## Skills Dispensadas (com justificativa)
- `ux-advanced/payment-checkout-ux` — esta fase não toca em pagamento; entra no sprint 4
- `ux-advanced/dark-mode-theming` — projeto não suporta dark mode (decisão em ADR-007)
```

**Example — bad citation (gate 3 blocks):**
...
→ Skill names not in catalog format. No `/` separator for category. Will fail.
...
- [ ] **Mandatory skills coverage verified (gate 3) — phase context matched against `triggers.yaml` of all 44 skills, missing citations reported as blockers**
