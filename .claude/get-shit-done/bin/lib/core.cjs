...
function getMilestoneInfo(cwd) {
  try {
    // SOURCE 1 (highest priority): MILESTONES.md "in_progress" entry
    // This is the authoritative source for current milestone in v0.8+.
    // ROADMAP.md headings can be ambiguous (multiple milestones listed),
    // and STATE.md frontmatter can be corrupted by previous bugged sessions.
    // MILESTONES.md is updated by milestone-complete commands and reflects
    // the current state of work. Multi-milestone projects (Rota Certa v1.1
    // example) need this to avoid being trapped on v1.0 status: completed.
    try {
      ...
      if (fs.existsSync(milestonesPath)) {
        ...
        // Look for table row with in_progress / ⏳ marker
        // Format: | MS-02 | v1.0 GA público | 2026-07-15 | 4,5,6 | ... | in_progress | OR ⏳
        const lines = milestonesContent.split('\n');
        for (const line of lines) {
          if (!line.includes('|')) continue;
          if (!/in_progress|in progress|⏳/i.test(line)) continue;

          const cells = line.split('|').map(c => c.trim()).filter(c => c.length > 0);
          if (cells.length < 2) continue;

          // Prefer semantic version (v1.1, v2.0.1) over ID format (MS-02)
          // Scan all cells looking for vN.N pattern first, then fallback to MS-NN
          let foundVersion = null;
          let foundName = null;

          for (let i = 0; i < cells.length; i++) {
            const verMatch = cells[i].match(/v(\d+(?:\.\d+)+)/i);
            if (verMatch) {
              foundVersion = 'v' + verMatch[1];
              // Name is the rest of the cell after stripping version, OR the next cell
              const nameInCell = cells[i].replace(verMatch[0], '').trim();
              foundName = nameInCell || (cells[i + 1] || 'milestone').trim();
              break;
            }
          }

          // Fallback: MS-NN format if no semantic version found
          if (!foundVersion) {
            for (let i = 0; i < cells.length; i++) {
              const idMatch = cells[i].match(/^(MS-\d+)$/);
              if (idMatch) {
                foundVersion = idMatch[1];
                foundName = (cells[i + 1] || 'milestone').trim();
                break;
              }
            }
          }
          ...
          if (foundVersion) {
            return { version: foundVersion, name: foundName || 'milestone' };
          }
        }
      }
    } catch { /* fall through to ROADMAP.md sources */ }

    // SOURCE 2: ROADMAP.md
    const roadmap = fs.readFileSync(path.join(planningDir(cwd), 'ROADMAP.md'), 'utf-8');

    // 2a. List-format roadmaps using 🚧 (in-progress) marker
    ...
    // 2b. Heading-format with 🚧 marker — "## Roadmap v1.1: Mobile + KYC" preceded
    // somewhere by 🚧 marker (in-progress) line
    // OR "## v1.1 Mobile 🚧"
    const headingWithMarker = roadmap.match(
      /(?:🚧[^\n]*\n[\s\S]*?)?##\s+(?:Roadmap\s+)?v(\d+(?:\.\d+)+)[:\s]+([^\n(🚧]+)\s*🚧/
    );
    if (headingWithMarker) {
      return {
        version: 'v' + headingWithMarker[1],
        name: headingWithMarker[2].trim(),
      };
    }

    // 2c. Heading where 🚧 appears AFTER the heading text (multiline scan)
    const lines = roadmap.split('\n');
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      const headingMatch = line.match(/^##\s+(?:Roadmap\s+)?v(\d+(?:\.\d+)+)[:\s]+([^\n(]+)/);
      if (headingMatch) {
        // Check this line and next 3 lines for 🚧 marker
    ...
    // 2d. Heading-format without explicit marker — strip <details>-wrapped shipped
    const cleaned = stripShippedMilestones(roadmap);
    // Extract version and name from the same ## heading for consistency
    // Supports 2+ segment versions: v1.2, v1.2.1, v2.0.1, etc.
    const headingMatch2 = cleaned.match(/##\s+(?:Roadmap\s+)?v(\d+(?:\.\d+)+)[:\s]+([^\n(]+)/);
    if (headingMatch2) {
      return {
        version: 'v' + headingMatch2[1],
        name: headingMatch2[2].trim(),
      };
    }
    // 2e. Fallback: try bare version match in cleaned content
