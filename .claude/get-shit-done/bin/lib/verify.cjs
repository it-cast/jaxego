 ...
 *
 * Antes da v0.9.6 o Gate 8 vivia apenas como prosa no workflow verify-phase.md
 * — "gate citado ≠ gate executado". Esta função torna o gate verificável por
 * script: a phase NÃO fecha se (a) QUALITY-BAR.md não existe, (b) existe
 * qualquer FAIL-BLOCK aberto, ou (c) FAIL-DEBT sem linha correspondente em
 * TECH-DEBT.md (a skill quality/senior-quality-bar exige contabilização).
 *
 * Formato esperado em QUALITY-BAR.md (gerado pelo verify-phase workflow):
 *   linhas de veredito contendo `FAIL-BLOCK`, `FAIL-DEBT`, `PASS` ou `N/A`.
 *   Um FAIL-BLOCK só é considerado resolvido se a MESMA linha contiver
 *   `[RESOLVIDO]` ou `RESOLVED`.
 */
function cmdVerifyQualityBar(cwd, phase, raw) {
  if (!phase) { error('phase required. Usage: gsd-tools verify quality-bar <phase>'); }
  const phaseInfo = findPhaseInternal(cwd, phase);
  if (!phaseInfo || !phaseInfo.found) {
    output({ passed: false, error: 'Phase not found', phase }, raw, 'failed');
    return;
  }
  ...
  // Localizar QUALITY-BAR.md (com ou sem prefixo numérico da phase)
  let qbFile = null;
  try {
    const files = fs.readdirSync(phaseDir);
    const match = files.find(f => /QUALITY-BAR\.md$/i.test(f));
    if (match) qbFile = path.join(phaseDir, match);
  } catch { /* phaseDir ilegível — cai no erro abaixo */ }

  if (!qbFile || !fs.existsSync(qbFile)) {
    output({
      passed: false,
      phase: phaseInfo.phase_number,
      quality_bar_file: null,
      errors: ['QUALITY-BAR.md ausente — Gate 8 não foi avaliado. A phase não pode fechar sem avaliação contra quality/senior-quality-bar.'],
      hint: 'Rode /gsd:verify-phase ' + String(phaseInfo.phase_number) + ' para gerar o QUALITY-BAR.md.',
    }, raw, 'failed');
    return;
  }

  const content = fs.readFileSync(qbFile, 'utf-8');
  ...
  for (const line of lines) {
    if (/FAIL-BLOCK/.test(line)) {
      if (!/\[RESOLVIDO\]|\bRESOLVED\b/i.test(line)) failBlocks.push(line.trim());
    } else if (/FAIL-DEBT/.test(line)) {
      failDebts.push(line.trim());
    } else if (/\bPASS\b/.test(line)) {
      passCount++;
    } else if (/\bN\/A\b/.test(line)) {
      naCount++;
    }
  }

  if (passCount === 0 && failBlocks.length === 0 && failDebts.length === 0) {
    errors.push('QUALITY-BAR.md existe mas não contém nenhum veredito (PASS / FAIL-BLOCK / FAIL-DEBT / N/A). Arquivo vazio ou template não preenchido não conta como gate avaliado.');
  }

  if (failBlocks.length > 0) {
    errors.push(`${failBlocks.length} FAIL-BLOCK aberto(s) — bloqueio absoluto, a phase não fecha:`);
    for (const fb of failBlocks.slice(0, 10)) errors.push(`  ✗ ${fb}`);
  }

  // FAIL-DEBT exige contabilização em TECH-DEBT.md
  if (failDebts.length > 0) {
    const tdPath = path.join(planningDir(cwd), 'TECH-DEBT.md');
    const tdContent = safeReadFile(tdPath) || '';
    const phaseRef = new RegExp(`phase[\\s-]*0*${escapeRegex(String(phaseInfo.phase_number))}\\b`, 'i');
    if (!tdContent || !phaseRef.test(tdContent)) {
      errors.push(`${failDebts.length} FAIL-DEBT registrado(s) no QUALITY-BAR.md mas TECH-DEBT.md não contém linha referenciando a phase ${phaseInfo.phase_number}. FAIL-DEBT sem contabilização é dívida invisível — exatamente o que o Gate 8 existe para impedir.`);
    }
  }
  ...
  output({
    passed,
    phase: phaseInfo.phase_number,
    quality_bar_file: path.relative(cwd, qbFile),
    verdicts: { pass: passCount, fail_block_open: failBlocks.length, fail_debt: failDebts.length, na: naCount },
    errors,
    warnings,
