...
<step name="gate_8_senior_quality_bar">
**Gate 8 — Senior Quality Bar (v0.9.5, bloqueante).**

Após reconcile e behavioral verification, avaliar a phase contra `quality/senior-quality-bar`:

1. Ler `.claude/skills/quality/senior-quality-bar/SKILL.md` (a Definição de Pronto).
2. Para cada bloco aplicável (A-dev sempre; B-segurança se toca endpoint/auth/PII/dados; C-deploy se há migração/release; D-UX se has_ui), classificar cada item: PASS | FAIL-BLOCK | FAIL-DEBT | N/A.
3. Coleta de evidência (não confiar em auto-relato):
   - Segredos: `grep -rIE "(api[_-]?key|secret|password|token)\s*[:=]\s*['\"][^'\"]{8,}" src/` deve vir vazio
   - N+1: endpoints de lista com relacionamento sem eager load → FAIL-BLOCK
   - Migração reversível: cada migration nova tem `downgrade` não-trivial
   - Auth: endpoint novo tem decisão explícita (decorator/dependency ou comentário "// público:")
   - Teste de erro: cada unidade nova tem ≥1 teste de caminho de erro
   - Fidelidade de wireframe (se UI-SPEC declara wireframe_source HTML): rodar
     `gsd-tools wireframe-contract <wireframe>` e conferir no CÓDIGO CONSTRUÍDO
     os elementos interativos do contrato (textos de botão, rotas de nav_targets,
     inputs por name/formControlName). Item do contrato ausente do código e
     ausente do bloco deviations do UI-SPEC → FAIL-DEBT classe UX (a tela
     entregue divergiu da fonte de verdade sem decisão registrada)
4. Escrever resultado em `QUALITY-BAR.md` da phase (tabela item × veredito × evidência).
5. **Validação por script (obrigatória, v0.9.6):** rodar
   `node .claude/get-shit-done/bin/gsd-tools.cjs verify quality-bar <N>`
   e confirmar `"passed": true`. Este é o enforcement real do Gate 8 — escrever o
   QUALITY-BAR.md sem rodar o check é gate citado, não gate executado. O mesmo
   check roda automaticamente no hook `gsd-phase-transition-guard.sh` antes de
   qualquer transição de phase, então pular este passo só adia o bloqueio.

**Bloqueio:** qualquer FAIL-BLOCK aberto → status `gaps_found`, phase não fecha. FAIL-DEBT → exige linha em TECH-DEBT.md citando a phase (o script valida isso); não bloqueia, mas dívida não contabilizada bloqueia.

Override só com `--skip-gate 8 --reason` em DECISIONS.md (confirmação dupla para segredo no repo e backup ausente).
</step>
...
- [ ] Gate 8 (Senior Quality Bar) evaluated; QUALITY-BAR.md written; no FAIL-BLOCK open
