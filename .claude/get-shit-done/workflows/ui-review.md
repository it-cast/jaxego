...
<objective>
Conduct 7-pillar visual + accessibility audit of Phase {phase_number}: {phase_name}
{If UI-SPEC exists: "Audit against UI-SPEC.md design contract."}
{If no UI-SPEC: "Audit against abstract 7-pillar standards."}

The 7th pillar is **Accessibility (WCAG 2.2 AA)** — score 0–4:
- 0: No accessibility considerations (alt text missing, no ARIA, no keyboard nav)
- 1: Partial — some alt text, basic ARIA but inconsistent
- 2: Adequate — most images have alt, form labels present, basic keyboard support
- 3: Good — ARIA live regions, focus management, 4.5:1 contrast in most places
- 4: Excellent — full keyboard nav, aria-label on all interactives, prefers-reduced-motion, 44×44px touch targets, 4.5:1 contrast everywhere, focus-visible styles
</objective>
